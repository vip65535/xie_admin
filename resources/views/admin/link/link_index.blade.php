            ...
            <div class="layui-tab-content clildFrame">
                <div class="layui-tab-item layui-show" style="padding:20px;">
                    ...
                    @endif
                        ...
                        <input hidden name="id" value="{{$link->id or ''}}" />
                                    <div class="layui-form-item">
                                        <label class="layui-form-label">名称</label>
                                        <div class="layui-input-block">
                                            <input type="text" style="width:400px;" name="name" value="{{$link->name or ''}}" class="layui-input newsName" lay-verify="required" placeholder="名称">
                                        </div>
                                    </div>
                                    <div class="layui-form-item">
                                        <label class="layui-form-label">链接</label>
                                        <div class="layui-input-block">
                                            <input type="text" style="width:400px;" name="href" value="{{$link->href or ''}}" class="layui-input newsName" lay-verify="required" placeholder="链接">
                                        </div>
                                    </div>
                                    <div class="layui-form-item">
                                        <label class="layui-form-label">站长邮箱</label>
                                        <div class="layui-input-block">
                                            <input type="text" style="width:400px;" name="email" value="{{$link->email or ''}}" class="layui-input newsName" lay-verify="required" placeholder="站长邮箱">
                                        </div>
                                    </div>
                                    <div class="layui-form-item">
                                            ...
                                            <div class="layui-input-block">
                                                <select name="type" class="newsLook"  lay-filter="browseLook">
                                                    @foreach(\App\Model\Link::$TYPE  as $k=>$v)
                                                        <option value="{{$k}}" <?php if(!empty($link)&&$k==$link["type"]){echo "selected";} ?> >{{$v['name']}}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                        </div>
